...
func TestCaesarPrepped(t *testing.T) {
	...
	for _, test := range caesarPrepped {
		if enc := c.Encode(test.pt); enc != test.ct {
			t.Fatalf("Shift Encode(%q) = %q, want %q.", test.pt, enc, test.ct)
		}
	}
}
...
func TestCaesar(t *testing.T) {
	testCipher("Shift", NewCaesar(), caesarTests, t)
}
...
func TestShift(t *testing.T) {
	// test shift(3) against Shift cases.
	c := NewShift(3)
	if c == nil {
	...
	// NSA and WP say Shift uses shift of -3
	c = NewShift(-3)
	if c == nil {
		t.Fatal("NewShift(-3) returned nil, want non-nil Cipher")
	}
